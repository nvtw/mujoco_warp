...
def check_aabb_overlap(
  box1_lower: vec3, box1_upper: vec3, box1_cutoff: float, box2_lower: vec3, box2_upper: vec3, box2_cutoff: float
) -> bool:
...
def nxn_broadphase(
  ...
  i = int((wp.sqrt(8 * elementid + 1) + 1) / 2)
  j = elementid - (i * (i - 1)) // 2
  pair = wp.vec2i(i, j)  # with i > j guaranteed
  geom1 = pair[0]
  geom2 = pair[1]

  if not proceed_broad_phase(collision_group[geom1], collision_group[geom2]):
    return
